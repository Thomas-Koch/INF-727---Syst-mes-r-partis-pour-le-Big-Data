	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		for (String line : a) {
			...
			if (retourOk) {
				...
				// On crée le dir benezeth
				commande = "ssh benezeth@" + line + " mkdir -p /tmp/benezeth";
				retourOk = ProcessLauncher(commande);
				
				if (retourOk) {
					// timeout pas atteint
					System.out.println(line + "Succes creation benezeth");
			
					// on copie SLAVE sur les machines
					commande = "scp SLAVE.jar benezeth@" + line + ":/tmp/benezeth/";
					retourOk = ProcessLauncher(commande);
					if (retourOk) {
						// timeout pas atteint
						System.out.println(line + "Succes copie SLAVE");
						
						// on envoie le fichier des machines 
						commande = "scp DEPLOY.txt benezeth@" + line + ":/tmp/benezeth/machines.txt";
						retourOk = ProcessLauncher(commande);
						if (retourOk) {
							// timeout pas atteint
							System.out.println(line + "Succes copie machines");

						} else {
							System.out.println(line + "Echec copie machines");
						}
					} else {
						System.out.println(line + "Echec copie SLAVE");
					}
				} else {
					System.out.println(line + "Echec creation benezeth");
				}
			} else {
